...
tlvpp::TLV::TLV(uint8_t *data, tlv_size_t data_length) {
  box = tlv_box_parse(data, data_length);
}
tlvpp::TLV::~TLV() { tlv_box_destroy(box); }
...
uint8_t *tlvpp::TLV::get_buffer() const { return tlv_box_get_buffer(box); }
...
void tlvpp::TLV::put(tlv_tag_t tag, uint64_t value) {
  tlv_box_put_longlong(box, tag, value);
}
void tlvpp::TLV::put(tlv_tag_t tag, int64_t value) {
  tlv_box_put_long(box, tag, value);
}
...
void tlvpp::TLV::put(tlv_tag_t tag, uint8_t *data, tlv_size_t data_length) {
...
uint64_t tlvpp::TLV::get_ulong(tlv_tag_t tag) const {
  int64_t out;
  tlv_box_get_long(box, tag, &out);
  return (uint64_t)out;
}
int64_t tlvpp::TLV::get_long(tlv_tag_t tag) const {
  int64_t out;
  tlv_box_get_long(box, tag, &out);
  return out;
}
...
void tlvpp::TLV::get_buffer(tlv_tag_t tag, uint8_t *data,
                            tlv_size_t *data_length) const {
